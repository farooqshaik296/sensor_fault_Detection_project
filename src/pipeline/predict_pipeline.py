...
class predictionPipline:
  ...
  def get_predicted_dataframe(self,input_dataframe_path: pd.DataFrame):
    ...
    try:
      ...
      predicton_column_name :str = TARGET_COLUMN
      input_df = pd.DataFrame = pd.read_csv(input_dataframe_path)

      input_dataframe = input_dataframe.drop(columns="Unnamed:0") if "Unammed:0" in input_dataframe.columns else input_dataframe

      prediction = self.predict(input_dataframe)

      input_dataframe[predicton_column_name] = [pred for pred in predictions]

      target_column_mapping = {0:'bad',1:'good'}

      input_dataframe[predicton_column_name] = input_dataframe[prediction_column_name].map(target_column_mapping)
